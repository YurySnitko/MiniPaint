...
export const Canvas: React.FC<CanvasRefProps> = observer(({ canvasRef }) => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      canvas.parentElement &&
        (canvas.width = canvas.height = canvas.parentElement.clientWidth);
  ...
  const finishDrawing: MouseEventHandler<HTMLCanvasElement> = ({
    nativeEvent,
  }) => {
    const ctx = contextRef.current;
  ...
  const draw: MouseEventHandler<HTMLCanvasElement> = ({ nativeEvent }) => {
    const ctx = contextRef.current;
    if (!isDrawing || !ctx || !newImageStore.chosenTool) {
      return;
    }
    const { offsetX, offsetY } = nativeEvent;
    ctx.strokeStyle = newImageStore.chosenColor;
